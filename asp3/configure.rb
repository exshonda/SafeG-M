...
OptionParser.new(nil, 22) do |opt|
  ...
  opt.on("--asp3-base BASE", "path of external vanilla ASP3 (overlay base; Step3)") do |val|
    $asp3base = val
  end
...
if $appldirs.empty?
  ...
  #  Step3: 既定の sample アプリは external(素ASP3) 側にもあるため ASP3_BASE/sample も探索する
  $appldirs.push("\$(ASP3_BASE)/sample")
end
$applname ||= "sample1"
...
#  Step3: cfg.rb は external(素ASP3)なので overlay 解決(SafeG所有→ASP3_BASE)で参照する．
$cfg ||= $ruby + " \$(call ovl,cfg/cfg.rb)"
$tecsgen ||= $ruby + " \$(TECSDIR)/tecsgen.rb"
#  Step3: 外部素ASP3(overlay base)の既定パス．未取り込みでも害が無いよう SRCDIR 相対で与える．
$asp3base ||= "\$(SRCDIR)/../asp3_base"
