...
### 確定再現手順・コミット
- **通しの確定手順（ビルド→配線→BOOTSEL→書込み→2シリアル取得）は `doc/rp2350_bringup.md` 末尾**
  「RP2350 NS=FreeRTOS（A）＋ NS 専用 UART1（B）確定再現手順」にコマンド付きで集約済。
- 実装コミット: asp3_core `b48b13a`(UART1 ブリングアップ+SAU R3)/`315cba2`(RESET_DONE 上限)/
  `a564514`(ACCESSCTRL パスワード=B 完成)、SafeG-M `a279c85`(NS UART1 直接駆動)。
- 配線: **GP8(物理ピン11)=UART1 TX → 別 USB-シリアル RX**、GND 共通、その tty=`/dev/ttyUSB0`(CP2102)。

### B 難航の経緯（教訓・要約）
当初 SAU だけで NS に UART1 を開放したが NS アクセスが BusFault(Excno=5)。原因は RP2350 が
SAU に加え **ACCESSCTRL でもペリフェラル S/NS をゲート**し、かつ **ACCESSCTRL 書込みに 0xACCE
パスワードが必須**(無しは無視され値が変わらない)だったこと。さらに Linux 側で **cat 前の
stty 115200 未設定**と **ModemManager の ttyACM 占有**が取得を阻害し切り分けを難しくした。
これらは §9 冒頭「★最重要の解決ポイント」「実機取得の教訓」に集約。
...
- `doc/rp2350_bringup.md` … RP2350 実機の確定再現手順（**末尾に baremetal A〜D1 9/9 と NS=FreeRTOS(A)+NS専用UART1(B) のコマンド付き通し手順**）
- `doc/m2_loadin_spec.md` / `doc/m3_nontecs_test.md` … 各フェーズ設計
