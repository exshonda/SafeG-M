...
5. **コードレビュー由来の TrustZone 分離ハードニング（2026-06-17 のレビューで検出、別タスク化）**。現状 9/9 green は維持されるが「安全グレードの分離保証」には未達の2点:
   - **#3 RP2350 ACCESSCTRL 設定**（High）: SAU に加え RP2350 の ACCESSCTRL で NS SRAM/ペリフェラルの S/NS を明示許可する。現状はリセット既定値依存（`asp3_core/target/pico2_arm_gcc/target_kernel_impl.c` の SAU 設定付近、コメントで認識済・未実装）。RP2350 固有レジスタ実装＋実機再検証が必要。
   - **#6 S→NS の FP レジスタ(s0-s31)内容 scrub**（Med）: BTASK FPスキップ修正は PSPLIM フォルトを解消したが Secure FP 残値の NS 漏洩は未対処。`launch_ns`（`asp3_core/.../core_kernel_impl.c`）の NS 呼出し直前に s0-s31/FPSCR を 0 クリアする案が低リスク。
   - 同レビューで検出した即修正可能な3点（CMSE gate ポインタ検証 / `set_control_ns` ISB / `TOPPERS_NS_VTOR` コンパイル時 assert）は **2026-06-17 修正済・実機 9/9 再確認済**。
