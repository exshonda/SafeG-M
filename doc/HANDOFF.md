...
- M4+: **実機RP2350 で NS=FreeRTOS も成立**(2026-06-17, `test/ns_freertos/pico2_arm_gcc` + `FreeRTOS/sample/pico2_arm_gcc`, ARM_CM33_NTZ port, A/B `SUMMARY total=5 pass=5`)。NS はベアメタル＋FreeRTOS の2種が RP2350 実機で確認済。
  - **A（NS 出力を Secure UART へ委譲）= 実機確認済**: gate `tg_puts`(NS ポインタ copy-in 検証)で FreeRTOS NS から `[NS] FreeRTOS NS console via Secure gate` を UART0(ttyACM0) に出力。
  - **B（NS 専用 UART1 直接駆動）= 実機確認済(2026-06-17)**: NS(FreeRTOS)が UART1(GP8)を Secure 非経由で直接駆動し `/dev/ttyUSB0` に `[NS-UART1] ...` 出力。鍵は ACCESSCTRL の NS 許可(書込みパスワード 0xACCE 必須)。下記 §9 参照。
...
## 9. B（NS 専用 UART1 直接駆動）= **完成（2026-06-17 実機確認済）**

**NS(FreeRTOS) が専用 UART1 を Secure を介さず直接駆動することを実機で確認**：
- UART1(GP8 TX)→別 USB-シリアル(CP2102=`/dev/ttyUSB0`)に
  `[NS-UART1] RP2350 NS=FreeRTOS direct console (Secure-brought-up UART1)` を出力。
- 同時に UART0(`/dev/ttyACM0`)で `[TST] SUMMARY total=5 pass=5`(A: gate 経由)も継続。
- 構成: Secure が UART1 を reset解除/clk_peri/GP8=TX,GP9=RX(funcsel2)/pad/baud までブリングアップ
  → SAU R3 で UART1 領域 0x40078000 を NS 属性化 → **ACCESSCTRL でバス層も NS 許可**。

### ★最重要の解決ポイント（#3 ACCESSCTRL）
- RP2350 はペリフェラルの NS アクセスを SAU に加え **ACCESSCTRL(0x40060000系)** でもゲートする。
  許可しないと NS の UART1 アクセスは **BusFault(Excno=5)** になる（SAU だけでは不足＝当初の「#3 不要」想定は否定）。
- **ACCESSCTRL レジスタ書込みは上位16bit に書込みパスワード `0xACCE` が必須**。無しの書込みは
  silently ignore され実機で値が変わらない(0xfc のまま)→ NS 拒否継続。これが B 難航の真因だった。
- 実装: `safeg_ns_uart1_init`(asp3_core `a564514`)で `*(0x400600A4) = 0xACCE0000 | 0xFF`(UART1 reg, 全許可)。

### 実機取得の教訓（Linux 側ハマり）
- **`cat` の前に必ず `stty -F <tty> 115200 raw -echo`**。Debugprobe VCP は host 設定 baud で
  UART0 をサンプリングするため、baud 未設定だと無音(Windows 端末は設定済なので見えた)。
- **ModemManager を止める**（`sudo systemctl stop ModemManager`）。CDC-ACM(ttyACM)を奪い取得を阻害。
  CP2102(ttyUSB)は影響を受けにくい。
- SAFEG=1 ファームは実走後 SWD ロック→書込み前に BOOTSEL(押しながら電源→離す)でクリーン化。
- このマシンの `cat` は前景・最小形(`timeout N cat /dev/tty > log`)が安定。`&`バックグラウンドは不安定。
- 連続出力のベアメタル UART1 チェック(`/tmp/uart1_check`, SAFEG=0)が結線・経路切り分けに有効だった。

---

### （旧）実装中の記録 — 参考
**当初コード実装・ビルド済、実機検証は第2シリアル配線待ちで中断していた**（以下は経緯記録）。

### 実装済みコミット（**未 push**）
- asp3_core `b48b13a`: `target/pico2_arm_gcc/target_kernel_impl.c` に `safeg_ns_uart1_init()`
  （Secure が UART1 を reset解除/clk_peri/GP8=TX,GP9=RX funcsel2/pad/baud 115200 までブリングアップ）
  ＋ **SAU R3 で UART1 データ領域 0x40078000..0x40078FFF を NS 化**。
- SafeG-M `a279c85`: `FreeRTOS/sample/pico2_arm_gcc/startup.c` に `ns_uart1_puts()`（UART1 DR/FR 直アクセス）。
  ResetISR で `[NS-UART1] RP2350 NS=FreeRTOS direct console ...` を UART1 へ出力。
- 設計: RESETS/CLOCKS/IO_BANK0/PADS は Secure 専有のまま、UART1 データ領域のみ NS 開放（分離は SAU 担保）。

### 必要な物理配線（このマシン固有・要・第2シリアル）
- **GP8（物理ピン11）= UART1 TX → USB-シリアル変換の RX**、**GND（例: 物理ピン13）→ アダプタ GND**。115200 8N1。
- Debugprobe VCP=`/dev/ttyACM0` は UART0 専用（Secure の [TST] 用）。UART1 はこの第2シリアルでのみ観測可能。
- アダプタ挿入後、増えた tty 番号（`ls /dev/ttyUSB* /dev/ttyACM*`）を確認すること。

### 検証手順（再開時）
1. Secure(SAFEG=1)+NS FreeRTOS をビルド（§4 と同手順。NS は `test/ns_freertos/pico2_arm_gcc`）。
2. **BOOTSEL でクリーン化**（SAFEG=1 ファームは実走後 SWD ロックするため。BOOTSEL 押下で電源再投入→離す）。
3. SWD で両イメージ書込み（reset しない）→ BOOTSEL 解放 → openocd `reset` 駆動。
   - **ライブ取得はこのマシンの background 制約で不安定**。ユーザー端末で `! cat /dev/ttyACM0`（UART0）と
     `! cat /dev/ttyUSB0`（UART1）を回し、電源再投入で取得するのが確実（実績）。
4. 期待: UART0 に `[TST] ... SUMMARY total=5 pass=5`、**UART1 に `[NS-UART1] ...`**（NS 直接駆動＝B 実証）。
5. **NS が UART1 アクセスでフォルトする場合** → RP2350 ACCESSCTRL(0x40060000系)の UART1 NS 許可を
   `safeg_ns_uart1_init()` 付近に追加（#3）。SAU のみで通れば ACCESSCTRL 既定が許可と判明＝#3 不要。

### 2026-06-17 検証セッションの状況（B 実機 green は未取得・要再開）
- 第2シリアル(CP2102)=**`/dev/ttyUSB0`** で配線(GP8→RX/GND)済。UART0=`/dev/ttyACM0`(Debugprobe VCP)。
- B ビルド書込みは成功(verify OK)するが、**UART0/UART1 とも [TST]/[NS-UART1] が取得できず**中断。
  切り分け結果:
  - SWD: ファーム実走で secure-debug ロック→ブレークポイント診断も不可(ロックが BP を無効化)。
  - `safeg_ns_uart1_init` の RESET_DONE 待ちは**上限付きに変更済(315cba2)**＝待ちループのハングは否定。
    それでも [TST] 不出のため、**Secure ハングではなく取得経路(Debugprobe VCP / 環境の cat)の不調**が最有力。
  - 本セッションは多数の BOOTSEL/openocd サイクルでシリアル取得が不安定化(background cat が exit 1、
    前景 cat も併用時に不発)。**Debugprobe と CP2102 の USB 再接続 + 素の vanilla 書込みで [TST] が出るか**を
    最初に確認するのが再開の早道(取得経路の健全性を素ファームで切り分け)。
- 未 push コミット(B): asp3_core `b48b13a`/`315cba2`、SafeG-M `a279c85`/`ed1eb4a`。A までは push 済。
- 再開時のクリーンな取得手順は §9 上記「検証手順」を踏襲。まず UART0 で [TST] が出る状態を回復してから
  UART1(`ttyUSB0`)の `[NS-UART1]` を確認する。
